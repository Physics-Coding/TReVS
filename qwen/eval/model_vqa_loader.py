...
def eval_model(args):
    ...
    print(f"[model_vqa_loader] Using seed={args.seed}", flush=True)
    ...
    print(f"[model_vqa_loader] Loading model: {model_path}", flush=True)
    print(f"[model_vqa_loader] model_kwargs={model_kwargs}", flush=True)
    ...
    if "device_map" not in model_kwargs and torch.cuda.is_available():
        model = model.cuda()
    model.eval()
    print(f"[model_vqa_loader] Model ready on device={model.device}", flush=True)

    min_pixels = 1280 * 28 * 28
    max_pixels = 1280 * 28 * 28
    fixed_height, fixed_width = get_fixed_image_size()
    print(
        f"[model_vqa_loader] Loading processor with fixed visual tokens=1280, resized={fixed_width}x{fixed_height}",
        flush=True,
    )
    processor = AutoProcessor.from_pretrained(model_path, min_pixels=min_pixels, max_pixels=max_pixels)
    print(
        f"[model_vqa_loader] Start eval: total={len(questions)}, chunk={args.chunk_idx}/{args.num_chunks}, answers={answers_file}",
        flush=True,
    )
    ...
    for i, line in enumerate(tqdm(questions, dynamic_ncols=True, mininterval=1.0)):
        ...
        if (i + 1) % progress_interval == 0 or (i + 1) == len(questions):
            log_progress("model_vqa_loader", i + 1, len(questions), answers_file)
    ans_file.close()
    print(f"[model_vqa_loader] Finished eval. answers={answers_file}", flush=True)
