...
def softmax_with_policy(attn, policy, eps=1e-6):
    """Mask policy-dropped keys while retaining every query's self edge."""
    B, N, _ = policy.size()
    B, H, T, N = attn.size()
    if T == 1:
...
def cluster_and_merge(x, cluster_num):
    ...
    gen = torch.Generator(device=density.device)
    gen.manual_seed(RANDOM_SEED)
    density = density + torch.rand(
        density.shape, device=density.device, dtype=density.dtype, generator=gen) * 1e-6
